"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
//Importacion que permite inyectar codigo en otros lados
var core_1 = require("@angular/core");
var ClothingService = (function () {
    function ClothingService() {
        this.listClothing = ['Camisa', 'Camiseta'];
    }
    ClothingService.prototype.testService = function (clothingName) {
        return clothingName;
    };
    //forza la devolucion con un tipo de dato explicito.
    ClothingService.prototype.addClothing = function (clothingName) {
        this.listClothing.push(clothingName);
        return this.getClothing();
    };
    ClothingService.prototype.getClothing = function () {
        return this.listClothing;
    };
    ClothingService.prototype.deleteClothing = function (index) {
        this.listClothing.splice(index, 1);
        return this.getClothing();
    };
    return ClothingService;
}());
ClothingService = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [])
], ClothingService);
exports.ClothingService = ClothingService;
//# sourceMappingURL=clothing.service.js.map